#Sur ma branche du package https://github.com/sanssouci-org/sanssouci/blob/guillermo-dev/R/tree-functions.R j'ai aussi mis deux nouvelles fonctions V.star.all.leaves.no.id et V.star.no.id plus rapides que V.star.all.leaves et V.star (j'ai supprimé du code inutile). Pouvez-vous vérifier que V.star.no.id donne bien les mêmes résultats que V.star ?
...
plot(comp_tp,
     ...
     main='Yeast Data',
     xlab='pval[1:n]')
lines(10*(1:aux)-comp_vstar,
      type='l',col='chocolate',lwd=3)
lines(10*(1:aux)-Vstar_noid,
      type='l',col='seagreen',lwd=3)
lines(10*(1:aux)-Vstar_allnoid,
      type='l',lty=2,col='coral1',lwd=3)
legend(x="bottomright", legend=c("Oracle","k-Vstar","k-Vstar.id","k-Vstar.a.no"), col=c("black",'chocolate',"seagreen","coral1"), lty=c(1,1,1,1))
