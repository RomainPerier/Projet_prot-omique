...
TP=cumsum(res_ordered[line_sorted_by_pval,]$Species!='ARATH')

df <- data.frame(Index=1:m,TP)
...
df <- cbind(df[1:(m%/%K)*K,],Vstar_DKWM,Vstar_HB,Vsimes,Vstar_refined)
...
save(df_plot,df,file='ARATH/save/df_plot.RData')
