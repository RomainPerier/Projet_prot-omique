...
verif_borne <- ZL_DKWM[[3]]==nb_pept
ind_levures <- c()
for (i in 1:length(split_data)){if (split_data[[i]][["Species"]][1] == "levure"){ind_levures<-rbind(ind_levures,i)}}
verif_lev <- verif_borne[ind_levures]
nb_lev <- sum(!verif_lev)
ind_violation_lev <- which(verif_borne[ind_levures] == FALSE)
lev_mauvaise_borne <- split_data[ind_violation_lev]
diff_est<- nb_pept[ind_violation_lev]-ZL_DKWM[[3]][ind_violation_lev]
...
save(df_plot,file='Reduced_tree/save/df_plot.RData')
